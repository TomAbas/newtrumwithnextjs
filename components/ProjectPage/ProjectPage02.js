...
const ProjectPage02 = ({ tagLine, tagLine1, projectsidx }) => {
  ...
  const animationWords = () => {
    ...
    let b = line1.map((word, idx) => {
      ...
      return (
        <h1 className={isArrWordIn ? `${styles.fadeInUp0}` : ""} style={delay}>
          {word}
        </h1>
      );
    });
  ...
  const animationWords1 = () => {
    ...
    let b = line2.map((word, idx) => {
      ...
      return (
        <h1 className={isArrWordIn1 ? `${styles.fadeInUp0}` : ""} style={delay}>
          {word}
        </h1>
      );
    });
    ...
    <>
      ...
      <div className={styles.newsPage02Box}>
        <div className={styles.newsTitleBox}>
          <div className={`${styles.newsTitle} ${styles.scaleAni}`}>
          ...
          <div className={styles.newsCredits}>
            <div
              className={`${styles.newsCreditsWrapper} ${styles.fadeTopDown}`}
            >
              <p>credits</p>
              <div className={styles.creditsBoxWrapper}>
                <div className={styles.creditsBox0}>
                  {/* <p>Directed by: LEE ES HOW</p>
            <p>Written by: LEE ES HOW & LEE ES WHO</p> */}
                  {/* <p>Produced by: item 7, Belga,HCM</p>
            <p>Producer: Audrey Pacard, SO HO</p> */}
                  {contributorList()}
                </div>
                <div className={styles.creditsBox1}>
                  {/* <p>Actors: Ho Van Le</p>
            <p>Sound design: KoanSound</p>
            <p>Motion capture studio: Studio YYY</p>
            <p>Additional animation: ZZZ Studio</p> */}
                </div>
              </div>
            </div>
          </div>
